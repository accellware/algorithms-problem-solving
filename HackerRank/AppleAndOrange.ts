...
function countApplesAndOranges(s: number, t: number, a: number, b: number, apples: number[], oranges: number[]): void {
    const numOfApples = apples.filter((applePosition) => (applePosition + a) >= s && (applePosition + a) <= t).length;
    
    const numOfOranges = oranges.filter((orangePosition) => (orangePosition + b) >= s && (orangePosition + b) <= t).length;
